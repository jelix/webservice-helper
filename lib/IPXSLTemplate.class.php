...
class IPXSLTemplate
{
    ...
    public function execute($model)
    {
        ...
        self::makeXML($model, $modelDom->documentElement);
    ...
    public static function makeXML($model, $parent, $addToParent = false)
    {
        if (is_array($model)) {
            foreach ($model as $name => $value) {
                if (!is_numeric($name)) {
                    ...
                    self::makeXml($value, $node, true);
                } else {
                    $node = $parent;
                    self::makeXml($value, $node);
                }
            }
            ...
            foreach ($model as $propertyName => $propertyValue) {
                ...
                self::makeXml($propertyValue, $property);
            }
        } else {
